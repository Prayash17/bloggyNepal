...
async function getDestinations(): Promise<Destination[]> {
  try {
    const data = await client.fetch<Destination[]>(destinationsQuery);
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#fbfaf7] text-slate-700">
      ...
      <section className="relative isolate overflow-hidden bg-slate-950 px-6 py-24 text-white sm:px-8 lg:py-32">
        ...
        <div className="relative mx-auto max-w-7xl text-center">
          ...
          <p className="mx-auto mt-7 max-w-2xl text-lg leading-relaxed text-white/75 sm:text-xl">
            Explore practical, honest destination guides with maps,
            itineraries, costs, travel logistics, and the details that make a
            journey smoother.
          </p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-20 sm:px-8 lg:py-28">
        <div className="mb-12 flex flex-col gap-4 border-b border-stone-200 pb-7 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="text-sm text-slate-500">
            {destinations.length}{" "}
            {destinations.length === 1 ? "guide" : "guides"} available
          </p>
        </div>
        ...
        {destinations.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-stone-300 bg-white px-6 py-20 text-center shadow-sm">
            ...
            <p className="mx-auto mt-3 max-w-md leading-relaxed text-slate-600">
              Your destination content exists in Sanity, but no published
              destination documents are currently being returned by the
              query.
            </p>
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {destinations.map((destination, index) => {
              const imageUrl = destination.coverImage
      ...
      <section className="bg-[#f1ede4] px-6 py-20 sm:px-8">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-7 rounded-2xl bg-white p-8 shadow-sm lg:flex-row lg:items-center lg:p-12">
          <div>
            ...
            <p className="mt-3 max-w-2xl leading-relaxed text-slate-600">
              Discover the people, landscapes, culture, and places worth
              knowing across every province.
            </p>
          </div>
